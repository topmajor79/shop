...
<%
/**
* @Class Name : egovProductList.jsp
...
<head>
	...
	<!--
		...
		function fn_egov_addView() {
			document.listForm.action = "<c:url value='/addProductView.do'/>";
			document.listForm.submit();
		}
		...
		function fn_egov_selectList() {
			...
			document.listForm.action = "<c:url value='/egovProductList.do'/>";
	...
	fetch('<c:url value="http://localhost:8081/api/shop/hello.do"/>')
	  .then(response => response.json())
	  .then(data => {
	      /* document.getElementById("test").innerHTML =
	          "message : " + data.message + "<br>" +
	          "status : " + data.status; */
	          let html = '<table style="font-size:10px; font-weight:normal;width:100px"';  
	          data.forEach(function(item) {
	              ...
	              html += "<td>" + item.useYn + "</td>";
	              html += "<td>" + item.regUser + "</td>";
	              html += "</tr>";
	          });
	          html += "</table>"; 
...
<div id="container" class="inner">
	...
	<!-- 타이틀 -->
	<h2 class="heading-large" id="test"><spring:message code="list.Product" /></h2>
	<!-- // 타이틀 -->
	
	<div id="content_pop">
		<form:form modelAttribute="ProductVO" id="listForm" name="listForm" method="post">
					...
					<div class="search-body">
						<div class="form-conts searchOption">
							<select id="searchCondition" name="searchCondition" class="krds-form-select medium" title="<spring:message code='search.choose'/>">
								<option value="1" <c:if test ="${not empty ProductVO.searchCondition and ProductVO.searchCondition eq 1}">selected="selected"</c:if>>Name</option>
								<option value="0" <c:if test ="${not empty ProductVO.searchCondition and ProductVO.searchCondition eq 0}">selected="selected"</c:if>>ID</option>
							</select>
						</div>
						<div class="form-conts btn-ico-wrap searchKeyword">
							<input type="text" id="searchKeyword" name="searchKeyword" value="${ProductVO.searchKeyword}" class="krds-input medium" placeholder="<spring:message code='search.keyword'/>">
			...
			<table class="tbl col data">
				...
				<thead>
					<tr>
						...
						<th scope="col" class="text-center"><spring:message code="title.Product.id" /></th>
						<th scope="col" class="text-center"><spring:message code="title.Product.name" /></th>
						<th scope="col" class="text-center"><spring:message code="title.Product.description" /></th>
						<th scope="col" class="text-center"><spring:message code="title.Product.useYn" /></th>
						<th scope="col" class="text-center"><spring:message code="title.Product.regUser" /></th>
					</tr>
				</thead>
				<tbody>
					<c:choose>
						<c:when test="${not empty resultList}">
							<c:forEach var="result" items="${resultList}" varStatus="status">
								<tr>
									<td class="text-center"><c:out value="${paginationInfo.totalRecordCount+1 - ((ProductVO.pageIndex-1) * ProductVO.pageSize + status.count)}"/></td>
